...
function Comments({
  ...
  useEffect(() => {
    ...
    const userProfile = JSON.parse(localStorage.getItem('user-profile-object'));
  ...
  const handleSubmit = (e) => {
    ...
    const commentObject = {
      ...
      pageId: Router.pathname, // pageID ref so we can group comments for Act page or Gig page
    ...
    let currentCacheProfile = JSON.parse(getFromCache('user-profile-object'));
    ...
    saveToCache('user-profile-object', json);
    // save to cache
    ...
    const isGig = Router.pathname.includes('gig');
    const isAct = Router.pathname.includes('act');
